...
def _dt_scale(state: dict) -> int:
    """Return the explicit time-step scale or its run-length-based default."""
    default = max(1, round(4.0 * float(state["simtime_days"]) / 10.0))
...
def build_generated_code(request: SimulationRequest) -> str:
    ...
    if (
        ...
        and not state.get("run_ambient_solution")
    ):
    ...
    if state.get("run_ambient_solution", False):
        lines.extend(
            [
                "# Preserve an identical, CME-free model for the ambient comparison run.",
                "ambient_model = copy.deepcopy(model)",
            ]
        )
    if state.get("chunked_solve", True):
        lines.append(
    ...
    if state.get("run_ambient_solution", False):
        lines.extend(
            [
                "",
                "# Evolve the comparison model with no CMEs.",
            ]
        )
        if state.get("chunked_solve", True):
            lines.append(
                f"s.solve_chunked(ambient_model, [], "
                f"chunk_simtime={float(state.get('chunk_size_days', 3.0))}*u.day)"
            )
        else:
            lines.append("ambient_model.solve([])")
    return "\n".join(lines) + "\n"
