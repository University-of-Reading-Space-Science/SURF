""" This script generates an offline ephemeris file for SURF."""
import datetime
...
def get_body_time_ranges():
    """
    Return a dictionary with the appropriate time ranges for each body.
    """
    bodies = get_naif_body_codes_dict()
    bodies = bodies.keys()

    # Set the default start and stop times for the ephemeris:
    t_start = Time('1963-01-01T00:00:00').to_datetime()
    t_stop = Time('2029-01-01T00:00:00').to_datetime()
    body_times = {b:{'start': t_start, 'stop': t_stop, 'step': '12H'} for b in bodies}

    # Now set the time ranges for spacecraft that don't fit the default.
    body_times['STA'] = {'start': Time('2007-01-01T00:00:00'),
                         'stop': get_sta_latest_horizons_date(),
                         'step': '3H'}

    body_times['STB'] = {'start': Time('2007-01-01T00:00:00'),
                         'stop': Time('2024-10-25T00:00:00'),
                         'step': '3H'}

    body_times['PSP'] = {'start': Time('2018-08-13T00:00:00'),
                         'stop': t_stop,
                         'step': '3H'}

    body_times['SOLO'] = {'start': Time('2020-02-11T00:00:00'),
                          'stop': t_stop,
                          'step': '3H'}

    body_times['ACE'] = {'start': Time('2006-07-03T00:00:00'),
                         'stop': get_ace_latest_horizons_date(),
                         'step': '3H'}

    body_times['ULYSSES'] = {'start': Time('1990-10-7T00:00:00'),
                             'stop': Time('2009-06-29T00:00:00'),
                             'step': '3H'}
...
def main():
    ...
    with SURF. This enables using SURF offline. The ephemeris includes the planets out to Saturn,
    and has a 6-hour cadence. It also includes the ephemeris for ACE, STEREO-A, STEREO-B, Parker
    Solar Probe, and Solar Orbiter, at 3-hour cadence. JPL Horizons only provides ephemeris data
    for ACE and STEREO-A for a short window ahead (roughly 70 days and 100 days, respectively). So
    the ephemeris file will need periodically updating if SURF is going to be used for case studies
    of new events or for forecasting.
    Returns:
    """

    root_dir = Path(__file__).parent.parent
    ephemeris_path = root_dir / 'data' / 'ephemeris' / 'ephemeris.hdf5'
    ...
    for body, naif_code in bodies_dict.items():
        ...
        for coord_sys in ['CARR', 'HEEQ', 'HAE']:
            ...
            if coord_sys == 'CARR':
                this_coord = body_coords.transform_to(
                    coords.HeliographicCarrington(observer="self"))
            elif coord_sys == 'HEEQ':
...
def get_sta_latest_horizons_date():
    """
    This function queries JPL Horizons to find the current latest available date of ephemeris data
    for STEREO-A. Typically, JPL Horizons only has 3-4 months of planned predicted ephemeris data
    for STEREO-A
    """
    # In the query, -234 is the NAIF code of STEREO-A. This query URL was given in an email exchange
    # with JPL Horizons
    # by Jon Giorgini.
    query = ("https://ssd.jpl.nasa.gov/api/horizons.api?format=text&OBJ_DATA='YES'"
             "&MAKE_EPHEM='NO'&COMMAND='-234'")
    response = requests.get(query, timeout=180)
    # This response is some paragraphs and a table. We want the date in the bottom right corner of
    # the table.
...
def get_ace_latest_horizons_date():
    """
    This function queries JPL Horizons to find the current latest available date of ephemeris data
    for ACE. Typically, JPL Horizons only has 2-3 months of planned predicted ephemeris data for ACE
    """
    # In the query, -92 is the NAIF code of ACE. This query URL was given in an email exchange
    # with JPL Horizons by Jon Giorgini.
    query = ("https://ssd.jpl.nasa.gov/api/horizons.api?format=text&OBJ_DATA='YES'"
             "&MAKE_EPHEM='NO'&COMMAND='-92'")
    response = requests.get(query, timeout=180)
    # This response is some paragraphs and a table. We want the date in the bottom right corner of
    # the table.
    lines = response.text.split("\n")
