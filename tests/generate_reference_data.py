...
def make_time_dependent_test_data():
    ...
    #  Add a CME
    cme = s.ConeCME(t_launch=0.5 * u.day, longitude=0.0 * u.deg, width=30 * u.deg, v=1000 * (u.km / u.s))
    cme_list = [cme]

    #  Setup HUXt to do a 5-day simulation, with model output every 4 timesteps (roughly half and hour time step)
    model_test = s.SURF(v_boundary=v_boundary, cr_num=2080, cr_lon_init=180 * u.deg, simtime=5 * u.day, dt_scale=4)
